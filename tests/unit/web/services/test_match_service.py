...
class TestThreeStagePipeline:
    ...
    @patch("web.backend.services.match_service.get_result_policy_store")
    @patch("web.backend.services.match_service.MatchLlmEvaluationService")
    def test_llm_metadata_can_be_included_without_changing_base_order(
        self,
        mock_judge_service_cls,
        mock_llm_policy_store,
        mock_store,
        ...
        results = service.get_matches(owner_id="owner-1", top_k=2, llm_ordering=False)

        assert [item.match_id for item in results] == ["m1", "m2"]
        assert [item.llm_rerank_score for item in results] == [55, 97]
        assert [item.llm_score for item in results] == [55, 97]
        assert all(item.llm_effective_for_rerank is False for item in results)
        assert all(item.llm_ignored_for_rerank_reason == "ordering_disabled" for item in results)
        assert service.last_llm_rerank_metadata["applied"] is False
        assert service.last_llm_rerank_metadata["eligible_count"] == 2
        assert service.last_llm_rerank_metadata["reason"] == "ordering_disabled"
...
class TestCoverageFocusedHelpers:
    ...
    def test_optional_attr_latest_eval_marker_and_job_detail_helpers(self, real_service, mock_db):
        ...
        source.site = "ats:00000000-0000-0000-0000-000000000123"
        source.job_url = "https://jobs.lever.co/example/abc"

        details = real_service._to_job_details(job)

        assert details.source_site == "ats:00000000-0000-0000-0000-000000000123"
        assert "refresh_availability" in details.availability_actions
        assert "refresh_unavailable" not in details.availability_actions
