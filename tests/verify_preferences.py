...
from core.matcher_service import MockMatcherService, PreferencesAlignmentScore
...
print(f"  Remote job: {score:.2f} (expected: 1.0) ✓" if score == 1.0 else f"  Remote job: {score:.2f} ✗")
...
print(f"  Tokyo job: {score:.2f} (expected: ~0.7) ✓" if score > 0.5 else f"  Tokyo job: {score:.2f} ✗")
...
print(f"  SaaS company: {score:.2f} (expected: 1.0) ✓" if score == 1.0 else f"  SaaS company: {score:.2f} ✗")

job_gaming = MagicMock()
job_gaming.company_industry = "Gaming"
score, _ = matcher.calculate_industry_match(job_gaming, preferences)
print(f"  Gaming company: {score:.2f} (expected: 0.0) ✓" if score == 0.0 else f"  Gaming company: {score:.2f} ✗")
...
print(f"  5 employees: {score:.2f} (expected: <0.5) ✓" if score < 0.5 else f"  5 employees: {score:.2f} ✗")

job_ideal = MagicMock()
job_ideal.company_num_employees = "100"
score, _ = matcher.calculate_company_size_match(job_ideal, preferences)
print(f"  100 employees: {score:.2f} (expected: 1.0) ✓" if score == 1.0 else f"  100 employees: {score:.2f} ✗")
...
print(f"  Software Engineer: {score:.2f} (expected: 1.0) ✓" if score == 1.0 else f"  Software Engineer: {score:.2f} ✗")

job_mgr = MagicMock()
job_mgr.title = "Engineering Manager"
job_mgr.job_level = "Senior"
score, _ = matcher.calculate_role_match(job_mgr, preferences)
print(f"  Manager: {score:.2f} (expected: 0.0) ✓" if score == 0.0 else f"  Manager: {score:.2f} ✗")
...
print("\n📊 Test 6: Scoring with Preferences")
boost, details = scorer.calculate_preferences_boost(alignment)
...
penalties, penalty_details = scorer.calculate_penalties(
    job_bad, 1.0, [], [], bad_alignment
)
