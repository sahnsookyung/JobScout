...
def get_match_details(match_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Get requirement matches with eagerly loaded requirement text
        req_matches = db.query(JobMatchRequirement).options(
            joinedload(JobMatchRequirement.requirement)
        ).filter(
            JobMatchRequirement.job_match_id == match_id
        ).all()
        
        # Debug logging to diagnose requirement text issues
        logger.info(f"Found {len(req_matches)} requirement matches for match {match_id}")
        for req in req_matches:
            has_req = req.requirement is not None
            req_text = req.requirement.text if has_req else None
            logger.info(f"  Requirement {req.job_requirement_unit_id}: has_requirement={has_req}, text={'present' if req_text else 'MISSING'}")
        
        # Check if the requirement units actually exist in the database
        if req_matches:
            unit_ids = [str(req.job_requirement_unit_id) for req in req_matches]
            from sqlalchemy import text
            result = db.execute(text("SELECT id, text FROM job_requirement_unit WHERE id IN :ids"), {"ids": tuple(unit_ids)})
            found_units = {str(row[0]): row[1] for row in result}
            logger.info(f"Direct SQL check - found {len(found_units)} requirement units: {found_units}")
        
        requirements = [
            RequirementDetail(
                requirement_id=str(req.job_requirement_unit_id),
                # Get requirement description from job_requirement_unit table via relationship
...
class PipelineStatusResponse(BaseModel):
    ...
    step: Optional[str] = None  # New: Granular step description
...
def _run_pipeline_background(task_id: str):
    ...
    try:
        ...
        def status_callback(step_name: str):
            with _pipeline_tasks_lock:
                if task_id in _pipeline_tasks:
                    _pipeline_tasks[task_id]["step"] = step_name
        
        # Create and store stop event
        stop_event = threading.Event()
        with _pipeline_tasks_lock:
             _pipeline_stop_events[task_id] = stop_event

        # Run the pipeline
        result = run_matching_pipeline(ctx, stop_event, status_callback=status_callback)
        
        # Update task with result
        with _pipeline_tasks_lock:
            if task_id in _pipeline_tasks:
                _pipeline_tasks[task_id]["status"] = "completed" if result.success else "failed"
                _pipeline_tasks[task_id]["result"] = result
                # Cleanup stop event
                _pipeline_stop_events.pop(task_id, None)
        ...
        try:
            _pipeline_controller.release_lock()
        except Exception as e:
            logger.error(f"Error releasing pipeline lock in cleanup: {e}")


@app.post("/api/pipeline/run-matching", response_model=PipelineTaskResponse, tags=["pipeline"])
def run_matching_pipeline_endpoint():
    ...
    """
    # 1. Check for existing running task within this process (fast check)
    with _pipeline_tasks_lock:
    ...
    # 2. Try to acquire global lock (cross-process check)
    # This prevents running if 'main.py' is executing
    if not _pipeline_controller.acquire_lock("frontend"):
        lock_info = _pipeline_controller.get_lock_info()
        owner = lock_info.get("source", "unknown") if lock_info else "unknown"
        
        return PipelineTaskResponse(
            success=False,
            task_id="",
            message=f"Pipeline is locked by another process ({owner}). Please try again later."
        )
...
@app.get("/api/pipeline/status/{task_id}", response_model=PipelineStatusResponse, tags=["pipeline"])
def get_pipeline_status(task_id: str):
    ...
    with _pipeline_tasks_lock:
        ...
        if result:
            response.matches_count = result.matches_count
            response.saved_count = result.saved_count
            response.notified_count = result.notified_count
...
@app.post("/api/pipeline/stop", response_model=PipelineTaskResponse, tags=["pipeline"])
def stop_matching_pipeline():
    ...
    with _pipeline_tasks_lock:
        ...
        # Signal stop
        if active_tid in _pipeline_stop_events:
            _pipeline_stop_events[active_tid].set()
            return PipelineTaskResponse(
                success=True,
                task_id=active_tid,
             ...
             return PipelineTaskResponse(
                success=False,
                task_id=active_tid,
                message="Could not find stop handle for active task."
            )
